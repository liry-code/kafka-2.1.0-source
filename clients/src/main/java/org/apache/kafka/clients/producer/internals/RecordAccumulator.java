...
public final class RecordAccumulator {
    ...
    public RecordAccumulator(LogContext logContext,
        ...
        this.batches = new CopyOnWriteMap<>();  // todo: java中JUC， 采用 “读写分离的思想”
     ...
     *
     * 分段加锁：
     * 在需要考虑多线程操作的情况下，线程的安全问题
     * 对于该加锁的地方一定要加锁，不需要加锁的地方不加锁
     *
     * 为了支持高并发场景下，既要保证线程安全性，也要保证代码的性能
     */
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
             ...
             * todo: 步骤一：获取消息分区获取队列
             */
            Deque<ProducerBatch> dq = getOrCreateDeque(tp);  // getOrCreateDeque() 线程安全
            synchronized (dq) {
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                // todo: 步骤二： 尝试向队列中添加消息
                /**
                 * 说明： 目前只是有了队列，数据是存储在批次中, 批次对象需要内存分配
                 * 目前还没分配到内存，所以添加数据会失败
                 */
            ...
            // todo: 步骤三: 计算批次的大小， AbstractRecords.estimateSizeInBytesUpperBound: 获取流大小
            /**
             * 在消息大小和 batchsize 之间取最大值作为当前的这个批次大小
             * 有可能一个消息比设定的batchsize 大， 默认一个批次大小16k， 那么就导致消息是一条一条的发送出去；
             * 这样的话，批次 也就没有意义
             * 所以，一定要根据自己的数据大小来合理设置批次参数大小
             */
            ...
            synchronized (dq) {
                ...
                // todo: 得到批次数据后，在再批次中添加数据才会成功
                FutureRecordMetadata future = Utils.notNull(batch.tryAppend(timestamp, key, value, headers, callback, time.milliseconds()));

                /**
                 * todo: 步骤六： 将批次添加到队列的队尾
                 */
                dq.addLast(batch);
    ...
    private Deque<ProducerBatch> getOrCreateDeque(TopicPartition tp) {
        ...
        if (d != null)
            return d;
        // todo: 如果队列不存在则为其新建一个空队列
        d = new ArrayDeque<>();
        // todo: putIfAbsent()添加键值对时，如果不存在则直接添加； 如果该key 存在对应的value 时，则返回该value ；
