...
public class Metadata implements Closeable {
    ...
    // todo: 更新元数据失败重试请求的最小时间间隔，默认值为100ms， 目的是减少网络压力
    private final long refreshBackoffMs;
    // todo: 多久更新一次元数据，默认为5分钟
    private final long metadataExpireMs;
    // todo: 元数据更新一次，版本号version就会自增1
    private int version;
    // todo: 上一次更新元数据时间
    ...
    // todo: 记录当前已存在的topic 和 过期时间 信息
    private final Map<String, Long> topics;
    // todo: metadata 更新时监听对象
    private final List<Listener> listeners;
    private final ClusterResourceListeners clusterResourceListeners;
    // todo: 是否需要将所有topic存储到元数据中
    ...
    public synchronized void update(Cluster newCluster, Set<String> unavailableTopics, long now) {
        ...
        // todo: topicExpiryEnabled默认为 true
        // todo: 第一次进来（KafkaProducer初始化时）时topic 为空，下面代码不执行
        // todo: 第二次进来（sender 方法）通过Producer的sender方法进来时，这个时候已经存在 topic 元数据信息
        ...
        // todo: needMetadataForAllTopics 默认值为false
        if (this.needMetadataForAllTopics) {
            ...
            // todo: 所有初始化的 时候，update这个方法没有去拉取元数据。
            // 直接把传进来的cluster对象赋值给 cluster ，内部只包含kafka集群地址
        ...
        // todo: 该方法最重要的作用是唤醒上一讲中wait线程，metadata.awaitUpdate(version, remainWaitMs)
        notifyAll();
