...
public class NetworkClient implements KafkaClient {
    ...
    private void doSend(ClientRequest clientRequest, boolean isInternalRequest, long now, AbstractRequest request) {
        ...
        // todo: 封装send对象， 封装了目标node 和 请求头
        Send send = request.toSend(destination, header);
        // todo: 基于clientRequest 构建 InFlightRequest 对象
        ...
        // todo: 将请求添加到 inFlightRequests
        this.inFlightRequests.add(inFlightRequest);
        // todo: 将send请求加入队列等待随后的poll 方法发送出去
        selector.send(send);
    }
    ...
    public List<ClientResponse> poll(long timeout, long now) {
        ...
        /**
         * todo: 3. 处理响应，响应中包含元数据信息
         */
        // process completed actions
        long updatedNow = this.time.milliseconds();
        List<ClientResponse> responses = new ArrayList<>();
        // todo: 处理发送完成的 send 对象
        ...
        // todo: 处理连接状态变化
        handleConnections();
        // todo: 处理 handleConnections 方法中标记 需要获取支持的api版本信息的节点
        handleInitiateApiVersionRequests(updatedNow);
        // todo: 处理超时请求
        handleTimedOutRequests(responses, updatedNow);
        // todo: 处理完成的 ClientResponse
        completeResponses(responses);
    ...
    private void handleCompletedReceives(List<ClientResponse> responses, long now) {
        // todo:  从completedReceives 中遍历所有接收到的响应， completedReceives的信息是在上一层中 selector.poll 中添加进去的
        for (NetworkReceive receive : this.selector.completedReceives()) {
            // todo: 响应节点id
            String source = receive.source();
            // todo: 从inFlightRequests缓存中 获取 request 对象
            InFlightRequest req = inFlightRequests.completeNext(source);
            // todo: 解析响应信息，验证响应头，封装 struct 对象
