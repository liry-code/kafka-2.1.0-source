...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    KafkaProducer(ProducerConfig config,
        ...
        try {
            ...
            //TODO: 压缩格式,默认为不压缩, <code>none</code>, <code>gzip</code>, <code>snappy</code>, <code>lz4</code>, or <code>zstd</code>.
            ...
            this.accumulator = new RecordAccumulator(logContext,
                    ...
                    apiVersions,
                    transactionManager,
                    // BATCH_SIZE_CONFIG: 默认为 16 kb
                    new BufferPool(this.totalMemorySize, config.getInt(ProducerConfig.BATCH_SIZE_CONFIG), metrics, time, PRODUCER_METRIC_GROUP_NAME));
            List<InetSocketAddress> addresses = ClientUtils.parseAndValidateAddresses(
                    config.getList(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG),
                    config.getString(ProducerConfig.CLIENT_DNS_LOOKUP_CONFIG));
            // TODO: 判断Kafka集群元数据是否存在
            if (metadata != null) {
                // metadata包含元数据信息，主要有kafka集群节点、topic信息、每个topic分区信息，以及分区对应的ISR列表分布情况
            ...
            //todo: 创建一个线程，传参 sender 对象
            // 实现把业务代码和线程代码进行隔离，显得代码清晰
    ...
    Sender newSender(LogContext logContext, KafkaClient kafkaClient, Metadata metadata) {
        ...
        /**
         * // todo: 初始化网络管理组件
         *  connections.max.idle.ms: 默认值为9分钟，一个网络连接最大可以空闲时间，超过这个空闲时间，则关闭网络连接
         *  设置为-1，不回收连接，减少频繁连接次数
         *
         *
         *  max.in.flight.requests.per.connection： 默认值为5，发送数据时，其实是有多个网络连接。每个网络连接可以 忍受producer端发送broker消息后，消息没有响应的个数；
         *  由于Kafka有重试机制，所有有可能造成数据乱序问题，要想保证数据有序，这个值需要设置为1；
         *
         */
        ...
        /**
         * acks:
         *  0: producer发送消息到broker之后就结束，不管消息是否发送成功失败
         *  1： producer发送消息到broker之后，并成功写入leader partition 之后返回响应
         *  -1 or all : producer发送消息到broker之后，成功写入leader partition 并等待数据同步到所有 follower partition之后返回响应
         */
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            ...
            try {
                // todo: 第一步： 同步等待拉取元数据，会触发网络请求
                // maxBlockTimeMs 最多等待时间，默认为 1 分钟
            ...
            // clusterAndWaitTime：表示目前拉取元数据使用了多长时间
            // remainingWaitMs： 表示拉取元数据 可使用的时间
            long remainingWaitMs = Math.max(0, maxBlockTimeMs - clusterAndWaitTime.waitedOnMetadataMs);
            // todo: 获取集群元数据
            Cluster cluster = clusterAndWaitTime.cluster;

            // todo: 第二步： 对key、value进行序列化
            byte[] serializedKey;
            try {
            ...
            // todo: 第三步：根据分区器选择消息所要发往的哪个分区
            int partition = partition(record, serializedKey, serializedValue, cluster);


            // todo: 第四步： 根据元数据封装分区对象
            tp = new TopicPartition(record.topic(), partition);

            setReadOnly(record.headers());
            Header[] headers = record.headers().toArray();

            // todo: 第五步1：判断消息大小是否超过消息最大字节数
            int serializedSize = AbstractRecords.estimateSizeInBytesUpperBound(apiVersions.maxUsableProduceMagic(),
                    compressionType, serializedKey, serializedValue, headers);

            // todo: 第五步2： 计算消息大小，并进行大小判断
            ensureValidRecordSize(serializedSize);
            long timestamp = record.timestamp() == null ? time.milliseconds() : record.timestamp();
            log.trace("Sending record {} with callback {} to topic {} partition {}", record, callback, record.topic(), partition);
            // producer callback will make sure to call both 'callback' and interceptor callback

            // todo: 第六步：给每个消息都绑定一个回调函数，因为采用异步请求方式发送消息
            Callback interceptCallback = new InterceptorCallback<>(callback, this.interceptors, tp);

            if (transactionManager != null && transactionManager.isTransactional())
                transactionManager.maybeAddPartitionToTransaction(tp);

            // todo: 第七步： 将消息添加到 缓冲区(默认为32M), 然后由 accumulator 将消息封装成一个批次的发送出去
            RecordAccumulator.RecordAppendResult result = accumulator.append(tp, timestamp, serializedKey,
                    serializedValue, headers, interceptCallback, remainingWaitMs);

            // todo: 如果是批次满了，或者是新创建一个批次  时 发送消息
            if (result.batchIsFull || result.newBatchCreated) {
                log.trace("Waking up the sender since topic {} partition {} is either full or getting a new batch", record.topic(), partition);

                // todo: 第八步：唤醒sender线程， 实现真正的消息发送
                this.sender.wakeup();
            }
     ...
     * @param topic The topic we want metadata for // todo: 要发送的topic名称
     * @param partition A specific partition expected to exist in metadata, or null if there's no preference // todo: 消息所对应的分片num
     * @param maxWaitMs The maximum time in ms for waiting on the metadata  // todo:最大等待时间
     * @return The cluster containing topic metadata and the amount of time we waited in ms
     * @throws KafkaException for all Kafka-related exceptions, including the case where this method is called after producer close
     */
    // todo: 连接kafka集群，获取topic 对应的元数据信息（topic,分区编号, 拉取数据最大时间）
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long maxWaitMs) throws InterruptedException {
        // add topic to metadata topic list if it is not there already and reset expiry
        //todo: 获取metadata中的cluster集群信息，由于刚开始进入该方法，所以cluster中只有address信息
        ...
        // todo: 将topic 添加到元数据对象中
        metadata.add(topic);


        // todo: 获取当前 topic 有多少分区
        Integer partitionsCount = cluster.partitionCountForTopic(topic);
        // Return cached metadata if we have it, and if the record's partition is either undefined
        // or within the known partition range
        // todo: 如果在元数据中获取到分区信息，说明本地有缓存元数据，那么直接返回cluster信息
        if (partitionsCount != null && (partition == null || partition < partitionsCount))
            // todo: 直接返回cluster元数据信息，并更新 拉取元数据时间
        ...
        do {
            ...
            metadata.add(topic);
            // todo: 获取 当前元数据版本号
            // todo: 更新 needUpdate 为 true
            int version = metadata.requestUpdate();
            // todo: 唤醒sender线程，开始执行拉取元数据操作，拉取动作由sender线程完成
            // todo: 涉及 java 线程、高并发
            sender.wakeup();
            try {
                // todo: 同步等待sender线程拉取元数据
    ...
    private int partition(ProducerRecord<K, V> record, byte[] serializedKey, byte[] serializedValue, Cluster cluster) {
        // todo: 如果指定了分区编号，则直接使用该分区编号; 否则使用分区器进行分区
